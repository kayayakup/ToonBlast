...
public class GoalPanel : MonoBehaviour
{
    ...
    public void SetupGoalObjects()
    {
        ...
        if (balloonGoal.Count > 0)
        {
            ...
            balloonGoal.SetSprite(LoadSprite("Sprites/Blocks/Balloon", "Balloon_0"));
        }
        if (duckGoal.Count > 0)
        {
            duckGoal.gameObject.SetActive(true);
            duckGoal.SetSprite(LoadSprite("Sprites/Blocks/Duck", "Duck"));
        }
        if (greenCubeGoal.Count > 0)
        {
            greenCubeGoal.gameObject.SetActive(true);
            greenCubeGoal.SetSprite(LoadSprite("Sprites/Blocks/CubeGreen", "CubeGreen"));
        }
        if (redCubeGoal.Count > 0)
        {
            redCubeGoal.gameObject.SetActive(true);
            redCubeGoal.SetSprite(LoadSprite("Sprites/Blocks/CubeRed", "CubeRed"));
        }
        if (blueCubeGoal.Count > 0)
        {
            blueCubeGoal.gameObject.SetActive(true);
            blueCubeGoal.SetSprite(LoadSprite("Sprites/Blocks/CubeBlue", "CubeBlue"));
        }
        if (yellowCubeGoal.Count > 0)
        {
            yellowCubeGoal.gameObject.SetActive(true);
            yellowCubeGoal.SetSprite(LoadSprite("Sprites/Blocks/CubeYellow", "CubeYellow"));
        }
        if (purpleCubeGoal.Count > 0)
        {
            purpleCubeGoal.gameObject.SetActive(true);
            purpleCubeGoal.SetSprite(LoadSprite("Sprites/Blocks/CubePurple", "CubePurple"));
        }

    }

    /// <summary>
    /// Sprite sheet'lerden de calisacak sekilde sprite yukler.
    /// Once direkt Load dener, bulamazsa LoadAll ile sprite adi eslestirilir.
    /// </summary>
    private Sprite LoadSprite(string path, string spriteName)
    {
        // Once tekil sprite olarak dene (single sprite mode)
        Sprite sprite = Resources.Load<Sprite>(path);
        if (sprite != null) return sprite;

        // Sprite sheet ise tum sprite'lari yukle, ismi eslestir
        ...
        // Eger hic eslesme yoksa ilk sprite'i don (varsa)
        if (sprites.Length > 0) return sprites[0];

        Debug.LogWarning($"GoalPanel: '{path}' yolunda '{spriteName}' sprite'i bulunamadi!");
        return null;
    }
