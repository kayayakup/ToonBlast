...
public class GridManager : MonoBehaviour
{
    ...
    public void UpdateArrays()
    {
        ...
        // Check if level has legacy balloon encoding (value 2 used for balloon when goal requires balloons but no balloons exist in grid)
        bool hasBalloons = false;
        bool hasLegacyBalloons = false;
        for (int i = 0; i < myGrid.GridSizeX; i++)
        {
            for (int j = 0; j < myGrid.GridSizeY; j++)
            {
                if (dataLevel.GameGrid.blockTypes[i].rows[j] == BlockTypes.Balloon)
                    hasBalloons = true;
                if (dataLevel.GameGrid.blockTypes[i].rows[j] == BlockTypes.Bomb)
                    hasLegacyBalloons = true;
            }
        }
        bool migrateLegacyBalloons = (myGoal != null && myGoal.balloonCount > 0 && !hasBalloons && hasLegacyBalloons);

        for (int i = 0; i < myGrid.GridSizeX; i++)
        {
            for (int j = 0; j < myGrid.GridSizeY; j++)
            {
                BlockTypes bt = dataLevel.GameGrid.blockTypes[i].rows[j];
                if (migrateLegacyBalloons && bt == BlockTypes.Bomb)
                {
                    bt = BlockTypes.Balloon;
                }
                blockTypes[i, j] = bt;
